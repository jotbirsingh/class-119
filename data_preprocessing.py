...
def create_bot_corpos(stem_words_classes):
...
# Tags-----------> as Label
stem_words_classes=create_bot_corpos(stem_words,classes)
...
for word_tags in pattern_word_tags_list:
    ...
    for word in stem_words:
        if words in pattern_words:
            bag_of_works.append(1)
        else:
            bag_of_works.append(0)
    labels_encoding=list(labels)
    tag=word_tags[1]
    tag_index=classes.index(tag)
    labels_encoding[tag_index]=1
    training_data.append([bag_of_works,labels_encoding])
